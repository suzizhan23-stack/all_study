<template>
  <div class="learning-page">
    <div class="learn-header">
      ...
      <div class="learn-progress">进度 {{ currentIndex + 1 }}/{{ queue.length }} <span class="badge badge-green">{{ correctCount }} 正确</span> <span class="badge badge-red">{{ wrongCount }} 错误</span></div>
    </div>
    ...
    <div class="card" style="margin-top:16px">
      <div class="learn-stats">
        <span>本次学习: 已学 {{ currentIndex + (answered ? 1 : 0) }} 词</span>
...
const mode = ref('card')
const queue = ref([store.sampleWord, { ...store.sampleWord, word: 'remarkable', meaning_cn: '显著的' }, { ...store.sampleWord, word: 'contribute', meaning_cn: '贡献' }])
const currentIndex = ref(0)
const flipped = ref(false)
const answered = ref(false)
const correctCount = ref(4)
...
<style scoped>
.learn-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px; flex-wrap: wrap; gap: 12px; }
.learn-mode-tabs { display: flex; gap: 4px; }
.learn-progress { font-size: 14px; color: var(--color-text-secondary); }
.learn-card { text-align: center; padding: 40px; }
...
.learn-stats { display: flex; gap: 20px; justify-content: center; font-size: 14px; color: var(--color-text-secondary); }
</style>
