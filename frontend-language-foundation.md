...
| **symbol** | 唯一的、不可变的值 | 每次 `Symbol()` 都不同；适合做对象属性键 |
...
#### 1.5.1 先看现代写法——你已经在用原型链了



```javascript

// 你用 class 写继承时，底层就是原型链
...
**问题来了：`dog.speak()` 是怎么找到 `speak` 这个方法的？**

- `dog` 对象本身只有 `name` 属性

- `speak` 和 `fetch` 不直接在 `dog` 上

- JS 去 `Dog.prototype` 上找 → 找到了 `speak`

- `toString` 连 `Dog.prototype` 上也没有 → 去 `Animal.prototype` 上找

- `hasOwnProperty` 再往上 → `Object.prototype` 上找
...
graph TB
    ...
    subgraph 查找_dog_speak

        DOG["dog 对象<br/>{ name: '旺财' }"]

        DOG -->|没有 speak?| DOG_PROTO["Dog.prototype<br/>{ speak, fetch }"]

        DOG_PROTO -->|也没有 toString?| ANIMAL_PROTO["Animal.prototype<br/>{ speak }"]

        ANIMAL_PROTO -->|也没有 hasOwnProperty?| OBJECT_PROTO["Object.prototype<br/>{ toString, hasOwnProperty }"]

        OBJECT_PROTO -->|还找不到?| NULL["null<br/>→ 返回 undefined"]
...
**这就是原型链：每个对象都有一个隐式链接指向另一个对象（它的"原型"），属性查找不到时就顺着这个链往上爬。**
...
graph TB
    ...
    subgraph 三个概念_别搞混
        ...
        ROLE1["① constructor（构造函数）<br/>你写的 function / class"]

        ROLE2["② prototype（构造函数的属性）<br/>new 出来的对象会继承它上面的方法"]

        ROLE3["③ __proto__（实例的属性）<br/>指向构造函数的 prototype"]
    ...
    subgraph 关系图
        ...
        CONS["Animal (构造函数)<br/>class Animal"] -->|".prototype"| CONS_PROTO["Animal.prototype<br/>{ speak }"]

        CONS_PROTO -->|".constructor"| CONS

        INST["dog (实例)<br/>new Animal()"] -->|".__proto__"| CONS_PROTO
...
// ① constructor：Animal 本身

Animal;                       // [class Animal]



// ② prototype：Animal 的一个特殊属性，存着所有实例共享的方法

Animal.prototype.speak;       // [Function: speak]

Animal.prototype.constructor; // [class Animal]（指向自己）



// ③ __proto__：实例上的隐藏属性，指向构造函数的 prototype
...
dog.__proto__.__proto__.__proto__  === null;           // true  ← 终点
...
| **`prototype`** | 构造函数（`class` / `function`） | 存共享方法的对象 | "类的蓝图" |

| **`__proto__`** | 所有对象（包括函数） | 自己的构造函数 的 `prototype` | "对象的归属证明" |

| **`constructor`** | `prototype` 上的属性 | 指向构造函数自己 | "我是谁造出来的" |



> 你可以把 `prototype` 理解成 **Java 的类方法表**，所有实例共享同一份。`__proto__` 就是实例指向这个表的指针。
...
// 这样 Dog 实例找不到的方法，会去 Animal.prototype 上找
...
graph TB
    ...
    subgraph 继承_原型链串联
        ...
        INST2["dog 实例<br/>{ name: '旺财' }"]

        INST2 -->|__proto__| DOG_PROTO2["Dog.prototype<br/>{ bark }"]

        DOG_PROTO2 -->|__proto__| ANIMAL_PROTO2["Animal.prototype<br/>{ speak }"]

        ANIMAL_PROTO2 -->|__proto__| OBJ_PROTO2["Object.prototype<br/>{ toString }"]

        OBJ_PROTO2 -->|__proto__| NULL2["null"]
    ...
    subgraph 方法查找路径
        ...
        PATH["dog.bark() → Dog.prototype → ✅ 找到"]

        PATH2["dog.speak() → Dog.prototype ❌ → Animal.prototype → ✅ 找到"]

        PATH3["dog.toString() → Dog ❌ → Animal ❌ → Object → ✅ 找到"]
...
obj.hasOwnProperty('a');      // true  ← a 是 obj 自己的属性

obj.hasOwnProperty('toString'); // false ← toString 是继承的



// 2. 获取对象的原型

Object.getPrototypeOf(dog) === Dog.prototype;  // true（推荐）

dog.__proto__ === Dog.prototype;               // true（不推荐，性能差）
...
obj2.greet();  // 'Hi'  ← obj2 本身没有 greet，从 proto 继承的
...
dog instanceof Animal;   // true（沿着原型链查）
...
#### 1.5.5 用 class 就行了，别折腾原型链



```javascript

// ✅ 现代写法：class（底层仍是原型链，但语法清晰）
...
> **架构师建议：** 理解原型链原理是为了调试和读源码，写代码时直接用 `class`。你不需要手动操作 `__proto__` 或 `prototype`，就像你不需要在 Java 里手动操作虚方法表一样。



#### 1.5.6 instanceof 的原理——就是走原型链



```javascript

// instanceof 的底层实现（伪代码）：
...
obj.greetNormal();  // Tom     ← this = obj

obj.greetArrow();   // undefined ← this = 全局（箭头函数定义时的外层）



// 箭头函数最常用的场景：回调里保留外层 this
...
const timer = {
  ...
  start() {
    ...
    setTimeout(function() {
      ...
      console.log(this.name);  // undefined（setTimeout 回调里的 this = 全局）

    }, 100);



    // ✅ 箭头函数：捕获 start 的 this

    setTimeout(() => {

      console.log(this.name);  // Timer（箭头函数保留了 start 的 this）
...
// Promise.all：全部成功才行，一个失败就全挂

// 类比: SQL 事务，要么全成功要么全回滚
...
| `Promise.all` | 全部成功才 resolve | 一个失败立即 reject | SQL 事务 | 并行请求，缺一不可 |

| `Promise.allSettled` | 全部完成才 resolve | 不 reject，返回每个的结果 | 批量发货 | 要每个请求的状态 |

| `Promise.race` | 第一个完成就 resolve/reject | 谁先失败就 reject | CDN 抢答 | 超时控制 |

| `Promise.any` | 第一个成功就 resolve | 全部失败才 reject | 备用服务器 | 多个资源任选一个 |
...
const proxy = new Proxy(user, {
  ...
  has(target, key) {
    ...
    if (key === '_secret') return false;  // 隐藏某些属性
...
#### 1.8.3 Reflect——默认行为的"副本"



```javascript

// Reflect 的作用：在 Proxy 里转发默认行为

// 每次 Proxy 拦截一个操作，Reflect 就提供对应的默认实现
...
const proxy2 = new Proxy(user, {
  ...
  get(target, key, receiver) {
    ...
    // Reflect.get 做的和 target[key] 一样

    // 但能正确处理一些边界情况（如继承、getter 的 this）
...
**为什么要用 Reflect？** 因为 `target[key]` 在处理 getter/setter 时可能丢失 `this` 指向，而 `Reflect.get/set` 能正确传递 `receiver` 参数。
...
// ─── CommonJS（默认，文件后缀 .js，package.json 无 type 字段） ───
...
// ─── ES Module（文件后缀 .mjs，或 package.json 中 "type": "module"） ───
...
**`dependencies` vs `devDependencies` 的区别——架构师必须会分：**

```bash
# dependencies：项目运行时需要
npm install react          # React / Vue / lodash / axios
# → 装到 dependencies，打包时会包含

# devDependencies：只在开发时需要
npm install -D typescript  # TypeScript / ESLint / Prettier / Vite / Jest
# → 装到 devDependencies，生产环境部署时用 npm ci --production 不会装
```
